...
from keycloak.authorization import Permission, Role, Policy
...
class KeycloakMiddleware(MiddlewareMixin):
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        try:
            user_permissions: AuthStatus = self.keycloak.has_uma_access(token, required_scope)
        except KeycloakInvalidTokenError as e:
            return JsonResponse({"detail": AuthenticationFailed.default_detail},
                                status=AuthenticationFailed.status_code)
        if user_permissions.is_authorized:
            return None
